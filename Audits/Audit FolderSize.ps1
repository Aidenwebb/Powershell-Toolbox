...
$ColumnDate = $SnapshotDate.ToString("yyyy/MM/dd")
$PathDate = $SnapshotDate.ToString("yyyy-MM-dd")

$OutputCSVPath = "C:\Logs\output-$PathDate.csv"
$ErrorLogPath = "C:\Logs\errors-$PathDate.log"


# Get complete list of folders to scan

foreach ($TargetFolder in $TargetFolders){
    If ($Targetfolder.Length -lt 260) {
        get-childitem $TargetFolder -Recurse -Directory | select -Property @{N='Path';E={$_.FullName}} | Get-STFolderSize | Select -Property @{N='Date';E={$ColumnDate}}, Path, TotalBytes | Export-Csv -Path $OutputCSVPath -NoTypeInformation -Delimiter '|' -Append
    }
    If ($Targetfolder.Length -ge 260) {
        "WARNING: Skipped due to too long path: $TargetFolder.Fullname" | Out-File -FilePath $ErrorLogPath -Append
        Write-Warning "Skipped due to too long path: $TargetFolder.Fullname"
    }
}